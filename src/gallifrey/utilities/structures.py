...
def find_closest(value_array:ArrayLike, reference_array:ArrayLike, 
                 is_sorted:bool=False,) -> NDArray:
    '''
    ...
    closest_index = np.where(np.abs(left_neighbour - value_array) 
                             <= np.abs(right_neighbour - value_array), 
                             indices-1, indices)
